...
class RecipeList(generic.ListView):
    """
    View to list all recipes with pagination.
    """
...
def add_recipe(request):
    """
    View to handle the addition of new recipes.
    """
...
def edit_recipe(request, slug):
    """
    View to handle recipe editing, restricted to the author.
    """
    recipe = get_object_or_404(Recipe, slug=slug)
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            messages.success(
                request, f'"{recipe.title}" updated successfully!'
            )
            return redirect("recipe_detail", slug=recipe.slug)
    else:
    ...
    return render(
        request, "center/edit_recipe.html", {"recipe": recipe, "form": form}
    )
...
def delete_recipe(request, slug):
    """
    View to handle recipe deletion, restricted to the author.
    """
    recipe = get_object_or_404(Recipe, slug=slug)
    if recipe.author != request.user:
